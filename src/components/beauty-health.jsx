...
const Beautyhelath = () => {
  ...
  const category = 'Beauty'; // Desired category name
  const Autogoods = () => {
    const [products, setProducts] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const category = 'Autogoods'; // Desired category name
    ...
    useEffect(() => {
      fetch(`http://localhost:5000/Products`)
        ...
        .then(data => {
          const filteredProducts = data.filter(product => product.cattegory === "Go'zallik va salomatlik");
